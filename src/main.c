...
// LED Blinkparameter
#define LED_BLINK_PERIOD 500000 // Ungefähr 0,5 Sekunden
...
uint16_t read_adc1_channel(uint8_t channel) {
    ...
    *ADC1_CR |= ADC_CR_ADSTART;  // Start conversion
    while (!(*ADC1_ISR & ADC_ISR_EOC));  // Wait for end of conversion
...
void space_vector_modulation() {
  ...
  if (v3 > v_max) v_max = v3;
  
  float v_min = v1;
  if (v2 < v_min) v_min = v2;
  if (v3 < v_min) v_min = v3;
  ...
  if (foc_state.duty1 > 0.95f) foc_state.duty1 = 0.95f;
  if (foc_state.duty2 > 0.95f) foc_state.duty2 = 0.95f;
  if (foc_state.duty3 > 0.95f) foc_state.duty3 = 0.95f;
  
  if (foc_state.duty1 < 0.05f) foc_state.duty1 = 0.05f;
  if (foc_state.duty2 < 0.05f) foc_state.duty2 = 0.05f;
  if (foc_state.duty3 < 0.05f) foc_state.duty3 = 0.05f;
...
void main(void)
{
    ...
    while (1) {
        ...
        // 1. Messen der Back-EMF Spannungen
        measure_bemf();
        
        // 2. Rotorwinkel aus Back-EMF schätzen
        estimate_rotor_position();
        
        // 3. Clarke Transform (bereits in estimate_rotor_position integriert)
        foc_state.alpha = bemf1_filtered;
        foc_state.beta = (bemf1_filtered + 2.0f * bemf2_filtered) / SQRT3;
        
        // 4. Park Transform - Umrechnung von stationärem in rotierendes Koordinatensystem
        float sin_phi = sinf(foc_state.phi);
        float cos_phi = cosf(foc_state.phi);
        foc_state.d = foc_state.alpha * cos_phi + foc_state.beta * sin_phi;
        foc_state.q = -foc_state.alpha * sin_phi + foc_state.beta * cos_phi;
        
        // 5. PID Controller
        // Geschwindigkeitsregelung: Drehzahlreferenz aus ADC in q-Sollwert umrechnen
        float speed_ref_filtered = (float)speed_ref / 4096.0f * 10.0f; // Skalieren auf 0-10
        pid_speed.setpoint = speed_ref_filtered;
        foc_state.q_ref = pid_calculate(&pid_speed, foc_state.omega);
        
        // Stromregelung d-Komponente (sollte nahe 0 sein für PMSM)
        pid_d.setpoint = foc_state.d_ref;
        float v_d = pid_calculate(&pid_d, foc_state.d);
        
        // Stromregelung q-Komponente (Drehmoment)
        pid_q.setpoint = foc_state.q_ref;
        float v_q = pid_calculate(&pid_q, foc_state.q);
        
        // PID-Ausgänge für Inverse Park Transform setzen
        foc_state.d = v_d;
        foc_state.q = v_q;
        ...
        // FOC-Status prüfen - wenn Geschwindigkeit und Strom innerhalb tolerabler Grenzen
        float q_error = fabsf(foc_state.q - foc_state.q_ref);
        float d_error = fabsf(foc_state.d - foc_state.d_ref);
        
        if (q_error < 0.2f && d_error < 0.1f && foc_state.phi != 0.0f) {
            foc_ok = 1; // FOC läuft korrekt
        } else {
            foc_ok = 0; // FOC hat Probleme
        }
        
        // Status-LED blinken lassen, wenn FOC korrekt läuft
        if (foc_ok) {
            // Blinken im Erfolgsfall
            ...
            // Statisch an, wenn Probleme auftreten
            *GPIOC_ODR |= (1 << LED_PIN_PC6);
        }
    }
}